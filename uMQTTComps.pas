...
uses
  Classes, uMQTT, OverbyteIcsWndControl, OverbyteIcsWSocket,
  OverbyteIcsWSocketS, Windows, Messages;
...
type
  ...
  public
    Subscriptions: TStringList; // 订阅的主题
  ...
  private
    Timers: HWnd;
    FGraceful: Boolean;
    FBroker: Boolean; // non standard，是否中介
  ...
  public
    Link: TWSocket; // 网络连接SOCKET
    Parser: TMQTTParser; // 解析器
  ...
  private
    ...
    procedure RxConnect(Sender: TObject; aProtocol: UTF8String; aVersion: byte;
      aClientID, aUserName, aPassword: UTF8String; aKeepAlive: Word; aClean: Boolean);
    procedure RxBrokerConnect(Sender: TObject; {non standard} aProtocol: UTF8String;
      aVersion: byte; aClientID, aUserName, aPassword: UTF8String; aKeepAlive: Word; aClean: Boolean);
    procedure SetMaxRetries(const Value: integer);
    procedure SetRetryTime(const Value: cardinal);
  public
...
function SubTopics(aTopic: UTF8String): TStringList;
begin
  result := __PartitionString(AnsiString(aTopic), '/', false, false);
end;
...
begin
  ...
  begin
    ...
    TMQTTServer(Owner.Owner).PublishToAll(Self, aMsg.Topic, aMsg.Msg, aMsg.Qos);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        bMsg.LastUsed := Now;
        Parser.SendPublish(aServer.NextMessageID, bMsg.Topic, bMsg.Msg, aQos, false, true);
      end;
    end;
  end;
...
begin
  ...
  Result.LocalBounce := false; // LocalBounce为False
                  ...
                  begin
                    ...
                    begin
                      ...
                      begin
                        ...
                        SetDup(bPacket.MsgStream, true);
                        aClient.DoSend(aClient.Parser, bPacket.ID, bPacket.Retries, bPacket.MsgStream);
                        bPacket.Free;
                      end
                      else
                      begin
...
begin
  ...
  begin
    ...
    else if (length(aClientID) < 1) or (length(aClientID) > 23) then // V3.1限制23个字节以内
    begin
...
begin
  ...
  begin
    ...
    if Assigned(FOnMsg) then FOnMsg(Self, aMsg.Topic, aMsg.Msg, aMsg.Qos, aMsg.Retained);
    Releasables.Remove(aMsg);
    aMsg.Free;
              ...
              begin
                ...
                begin
                  ...
                  begin
                    ...
                    mon('Re-issuing Message ' + inttostr(bPacket.ID) + ' Retry ' + inttostr(bPacket.Retries));
                    SetDup(bPacket.MsgStream, true);
                    DoSend(Parser, bPacket.ID, bPacket.Retries, bPacket.MsgStream);
                    bPacket.Free;
                  end
                  else
                  begin
...
begin
  ...
  Result.MsgStream.CopyFrom(aMsg, aMsg.Size);
  List.Add(Result);
end;
...
begin
  ...
  MsgStream.Clear;
  From.MsgStream.Seek(0, soFromBeginning);
  MsgStream.CopyFrom(From.MsgStream, From.MsgStream.Size);
